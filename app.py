...
def wordcount_score(words, min_w, max_w):
    ...
    if min_w: return 1.0 if words >= min_w else max(0.0, 1 - (min_w - words)/max(1,min_w))
    if max_w: return 1.0 if words <= max_w else max(0.0, 1 - (words - max_w)/max(1,max_w))
    return 1.0

def semantic_similarity(desc, transcript):
    if not desc or not transcript: return 0.0
